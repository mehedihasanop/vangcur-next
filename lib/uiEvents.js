...
// Legacy: genInvoice(o) — 32-javascript-all.js line ~9399 (no numbered 01-42 section
// file exists for this — legacy handled it as a window.open() target with an inline-
// modal fallback, not a standalone .html section). Converted (2026-08-01) as
// app/components/modals/InvoiceModal.js, mounted globally in GlobalOverlays.js.
// detail: { orderId, order, ctx? } — order is included when the caller already has
// the full object in hand (e.g. BgConfirmPopup.js), so InvoiceModal.js doesn't need a
// redundant Supabase fetch; order may be omitted otherwise (InvoiceModal.js then
// fetches by orderId itself, Supabase first then vc_orders localStorage fallback,
// same as legacy's dlInvoiceById()). `ctx` mirrors legacy's dlInvoiceById(id,
// callerCtx)/invBackBtn's callerCtx check (~9608-9631): ctx:'track' (OrderTracking.js)
// re-opens OrderTracking on close, ctx:'acc' (AccountPage.js) re-opens AccountPage via
// OPEN_ACCOUNT_EVENT, omitted (BgConfirmPopup.js/WaitingPage.js) means "fresh
// post-checkout invoice" — the default branch that dispatches SHOW_POST_ORDER_INFO_EVENT.
export const GENERATE_INVOICE_EVENT = 'vc:generateInvoice';

// Legacy: membershipCardBtn's onclick -> openMembershipModal() — 32-javascript-all.js
// lines ~500-540 (39-membership-progress-modal.html). AccountPage.js's stat-box
// dispatches this; detail: { completedCount } carries the same confirmed-order count
// legacy read from membershipCardBtn.dataset.completed (here: stats.completed from
// lib/accountData.js's orderStats()). MembershipModal.js (app/components/modals/,
// mounted globally in GlobalOverlays.js) owns the open/close state itself.
...
// Legacy: invBackBtn onclick's default branch (32-javascript-all.js ~9626-9630) —
// after InvoiceModal.js closes, if ctx is neither 'acc' nor 'track' (i.e. this was a
// fresh post-checkout invoice, not one reopened from the account page or order
// tracking), it shows #postOverlay. PostOrderInfo.js (27-post-order-info.html)
// listens for this event; InvoiceModal.js dispatches it in that default branch.
// No detail needed.
