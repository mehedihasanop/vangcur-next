...
export async function updateProfileName(supabase, currentUser, newName) {
  try { const { error } = await supabase.auth.updateUser({ data: { name: newName } }); if (error) throw error; } catch (e) { logWarn('[Vangcur] auth.updateUser:', e); }
...
// ── Stock notification list (legacy _renderSNProfileSection). Written by
//    lib/stockNotifyData.js's submitStockNotify(), called from
//    app/components/modals/StockNotifyModal.js (2026-08-01) ──
