...
// - getWishlist/saveWishlist/isWishlisted/toggleWish (legacy lines ~7059-7081). Stays
//   localStorage-only here on purpose: the Supabase cross-device sync half now lives in
//   app/components/auth/LoginModal.js, which listens for WISHLIST_EVENT and pushes to
//   Supabase whenever a user is signed in (see lib/authData.js's file-header note for why
//   it's there instead of here — avoids a circular import).
    ...
    try {
      ...
      if (error) {
        ...
        if (error.code === '42501' || error.code === 'PGRST116' || error.message?.includes('permission') || error.message?.includes('policy')) {
          logError('[Vangcur] ❌ RLS/Permission error — custom_products টেবিলে anon SELECT access নেই।');
          return [];
        }
...
//
// Channel name includes a random suffix — supabase-js throws
// "tried to subscribe multiple times" if two channels share the exact same
// topic name and are subscribed at the same time. This function used to hand
// out the same hardcoded 'products-grid-watch' name to every caller; that was
// fine while only ProductGrid.js called it, but broke production the moment a
// second always-mounted component (an earlier overlay version of the search
// page) called it too on every home-page load — both tried to subscribe to
// the identical channel simultaneously and it crashed the whole page. Unique
// names per call make this safe no matter how many components subscribe at once.
...
// Legacy: toggleWish(id, btn) (32-javascript-all.js ~10614-10628) — DOM-mutation
// half removed, caller (ProductCard/ProductDetailClient) owns its own UI state.
// BUG FIX (2026-08-01): the showToast() calls legacy always made on both branches
// ('❤️ Wishlist এ যোগ হয়েছে!' / 'Wishlist থেকে সরানো হয়েছে') were dropped somewhere
// during the conversion — saveWishlist()/WISHLIST_EVENT still fired correctly (so the
// heart icon and wishlist count always updated), but no popup ever appeared, on
// every route that calls this (ProductCard.js on the home page, ProductDetailClient.js
// on /product/[slug]). "কার্টে যোগ করুন" was never affected — that toast lives in
// CartSidebar.js's own QUICK_CART_EVENT listener, a completely separate code path.
export function toggleWish(prod) {
  ...
  saveWishlist(w);
  // Legacy: showFloatWish() only ever fires from toggleWish()'s add branch (never on
  // remove) — mirrors CART_ADD_EVENT's fire-only-on-add convention in cartData.js.
  // FloatWishBadge.js (36-floating-wishlist-badge.html) is the only listener.
...
// QUICK_ORDER / QUICK_CART have no listener yet — their overlays (23-order-overlay,
// 20-cart-sidebar) are still ⏳ বাকি per the master prompt's progress table.
// STOCK_NOTIFY_EVENT's listener (2026-08-01) is app/components/modals/StockNotifyModal.js,
// mounted globally in GlobalOverlays.js.
...
// Legacy: showQuickOrderModal() (32-javascript-all.js ~7791) — the "🛒 শপিং কার্ট"
// bottom-sheet shown by orderNow()/quickOrder() when the cart already had items
// before this click. QuickOrderBridge.js dispatches this (no payload — the modal
// re-reads the cart itself, same pattern CartSidebar.js uses); QuickOrderModal.js
// (2026-08-01) is the listener.
export const QUICK_ORDER_MODAL_EVENT = 'vc:quickOrderModal';
export const STOCK_NOTIFY_EVENT = 'vc:stockNotify';

// Legacy: makeSlug(str) (32-javascript-all.js line ~6) — used by openPP()'s
// history.replaceState('/'+slug) branch. Now that 19-product-full-page.html is a
// real route (app/product/[slug]/, owner's decision — see that route's page.js
// comment) instead of the legacy pp-overlay, this is what builds the actual URL.
...
// Legacy: findProdBySlug() built the slug on demand every time a card was opened;
// here we precompute the href once per product. Format matches legacy exactly:
// `${makeSlug(name)}-${id}` so an old-style bookmarked/shared URL still resolves
// (app/product/[slug]/page.js accepts either this full slug or a bare id).
export function productHref(prod) {
  return `/product/${makeSlug(prod.name)}-${prod.id}`;
}

// Legacy: findProdBySlug(slug) (32-javascript-all.js line ~8) — id-only, exact-slug,
// or slug-ends-with-"-id" fallback, in that order. Used by app/product/[slug]/page.js
// and ProductDetailClient.js to resolve whichever form of the URL was requested.
export function findProdBySlug(prods, slug) {
  if (!slug) return null;
...
// Legacy: extracts the trailing numeric id from a slug like "some-product-name-14"
// (or returns the slug itself if it's already a bare id) — used server-side by
// generateMetadata()/the page component before the full PRODS list is available.
