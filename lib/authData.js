...
// - _getLinkedAccounts/_saveLinkedAccounts/_saveLinkedAccount (lines ~2040-2059) — kept
//   for 22-account-page.html's account-switcher (Priority 2, not built yet).
//   switchToAccount() itself (lines ~2060+) belongs there, not here.
...
// ── Linked accounts — multi-account switcher storage (22-account-page.html reads this;
//    switchToAccount() itself is that page's job, not this file's) ──
...
export async function signInWithGoogle(supabase) {
  const redirectTo = typeof window !== 'undefined' ? window.location.origin : 'https://vangcur.netlify.app';
...
export async function saveWishlistToSupabase(supabase, userId, items) {
  ...
  } catch (e) { console.warn('[Vangcur] wishlist Supabase sync failed:', e); }
}
...
export async function mergeGuestOrdersToUser(supabase, userEmail, userId) {
  try {
    ...
    if (orderNums.length) {
      ...
      if (error) console.warn('[Vangcur] guest order merge error:', error);
    }
  } catch (e) { console.warn('[Vangcur] mergeGuestOrdersToUser exception:', e); }
    }
